...
void *procesaPeticion(petinfo *pet)
{
    ...
    case 'L':
        ...
        ptr = strtok_r(NULL, " \t\n", &saveptr); //FASE 3
        poslista2 = atoi(ptr); 
        if ((poslista1 < 0 || poslista1 >= pet->tamlista) || (poslista2 < 0 || poslista2 >= pet->tamlista)) //FASE 3
            valido = 0;
        else {
            // esperar a tener los mutex que protegen los recursos solicitados
            pthread_mutex_lock(&pet->lista[poslista1]); //FASE 3
            pthread_mutex_lock(&pet->lista[poslista2]); //FASE 3
        }
        break;
    case 'u':
    case 'U':
        ...
        if ((poslista1 < 0 || poslista1 >= pet->tamlista) || (poslista2 < 0 || poslista2 >= pet->tamlista)) //FASE 3
            ...
            // desbloquear los mutex que protegen los recursos que se liberan
            pthread_mutex_unlock(&pet->lista[poslista1]);
            pthread_mutex_unlock(&pet->lista[poslista2]);
        }
        break;
    default:
        valido = 0;
